...
Usage:
------
    python examples/train_dqn.py --episodes 50000 --eval-freq 1000
"""
...
class TrainingMetrics:
    ...
    def add_unique_state(self, state: np.ndarray):
        """Add a state to the unique states set."""
        # Convert state to hashable tuple
        state_tuple = tuple(state.flatten())
        self.unique_states_set.add(state_tuple)
...
def train_dqn_agent(
    ...
    print(f"  Health check frequency: every {health_check_freq} episodes")
    ...
    for episode in range(1, num_episodes + 1):
        # Play self-play game (track states every 10 episodes to avoid memory issues)
        track_states = (episode % 10 == 0)
        ...
        if episode % eval_freq == 0:
            print(f"\n📊 Episode {episode}/{num_episodes} - Evaluating...")
            ...
            print(f"  Unique states explored: {len(metrics.unique_states_set):,}")
