# Same as audit_dqn_clean_with_tracking.py but with 50 iterations instead of 10
...
print("="*80)
print("AUDIT: Extended Training (50 iterations)")
print("="*80)
print()
...
print("After Player -1 plays column 6:")
env.render()
print()
...
print("After Player 1 plays column 3 (WINS!):")
env.render()
print()

# ============================================================
# TEST WINNER EXPERIENCE (50 iterations)
# ============================================================

print("="*80)
print("WINNER EXPERIENCE (50 iterations)")
print("="*80)
...
# ============================================================
# TEST LOSER EXPERIENCE (50 iterations)
# ============================================================

print("="*80)
print("LOSER EXPERIENCE (50 iterations)")
print("="*80)
...
# Winner Q progression
ax = axes[0, 0]
ax.plot(range(len(winner_q_progression)), winner_q_progression, marker='o', markersize=3, linewidth=2)
ax.axhline(y=1.0, color='r', linestyle='--', linewidth=2, label='Target (+1.0)')
ax.axhline(y=winner_q_progression[0], color='g', linestyle=':', alpha=0.5, label='Initial')
ax.set_xlabel('Iteration')
ax.set_ylabel('Q-Value')
ax.set_title('Winner Q-Value Progression')
ax.legend()
ax.grid(True, alpha=0.3)

# Winner loss
ax = axes[0, 1]
ax.plot(range(1, len(winner_losses)+1), winner_losses, marker='o', markersize=3, linewidth=2, color='orange')
ax.set_xlabel('Iteration')
ax.set_ylabel('Loss')
ax.set_title('Winner Loss (should decrease)')
ax.set_yscale('log')
ax.grid(True, alpha=0.3)

# Loser Q progression
ax = axes[1, 0]
ax.plot(range(len(loser_q_progression)), loser_q_progression, marker='o', markersize=3, linewidth=2, color='purple')
ax.axhline(y=-1.0, color='r', linestyle='--', linewidth=2, label='Target (-1.0)')
ax.axhline(y=loser_q_progression[0], color='g', linestyle=':', alpha=0.5, label='Initial')
ax.set_xlabel('Iteration')
ax.set_ylabel('Q-Value')
ax.set_title('Loser Q-Value Progression')
ax.legend()
ax.grid(True, alpha=0.3)

# Loser loss
ax = axes[1, 1]
ax.plot(range(1, len(loser_losses)+1), loser_losses, marker='o', markersize=3, linewidth=2, color='brown')
ax.set_xlabel('Iteration')
ax.set_ylabel('Loss')
ax.set_title('Loser Loss (should decrease)')
ax.set_yscale('log')
ax.grid(True, alpha=0.3)
...
print("\nPlots show Q-value and loss progression over 50 iterations")